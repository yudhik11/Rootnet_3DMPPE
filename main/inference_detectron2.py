...
import os, json
#python inference.py --gpu 0 --test_epoch 18 --image_id dir --annotations file
#python inference_detectron2.py --gpu 0 --test_epoch 18 --annotations /home/yudhik/detectron2/temp/annotations/ch_2/annotations.json --image_dir /home/yudhik/detectron2/temp/data/ch_2 --out ch_2.npy
def parse_args():
    ...
    parser.add_argument('--annotations', type=str, dest='annotations')
...
def main():
    ...
    tester._make_model()
    bboxes = get_bbox(args.annotations)
    depths = []
    from utils_plt import plt_lines
    with torch.no_grad():
        # for itr, (input_img, cam_param) in enumerate(tqdm(tester.batch_generator)):
        for filename in tqdm(sorted(os.listdir(args.image_dir))):
            fl = os.path.join(args.image_dir, filename)
            #print(len(bboxes[filename]['boxes']))
            #exit()
            fr=[]
            for kk in range(0, len(bboxes[filename]['boxes'])):
                bbox = bboxes[filename]['boxes'][kk]
                bbox[2] = bbox[2]-bbox[0]
                bbox[3] = bbox[3]-bbox[1]
                input_img, cam_param, bbox, c = get_item(fl, bbox)
                ...
                bbox, pred_root = evaluate(coord_out, bbox, c)    
                fr.append(pred_root)
            depths.append(np.array(fr))
    depths = np.array(depths)
    print(depths.shape)
    if args.out:
        np.save(args.out, depths)
    else:
        print("no output")
        plt_lines(data=depths)
        #y = np.arange(0, depths.shape[0])
        #plt.plot(y, depths[:, 2])
        #plt.show()
